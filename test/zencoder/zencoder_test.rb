...
class ZencoderTest < Test::Unit::TestCase
  ...
  should "encode to xml" do
    assert_match /<api-request>/, Zencoder::Serializer.encode({:api_request => {:input => 'https://example.com'}}, :xml)
  end

  should "encode to json" do
    assert_match /"api_request"/, Zencoder::Serializer.encode({:api_request => {:input => 'https://example.com'}}, :json)
  end
  ...
  should "decode from xml" do
    assert Zencoder::Serializer.decode("<api-request><input>https://example.com</input></api-request>", :xml)['api_request']['input']
  end

  should "decode from json" do
    assert Zencoder::Serializer.decode(%@{"api_request": {"input": "https://example.com"}}@, :json)['api_request']['input']
  end
